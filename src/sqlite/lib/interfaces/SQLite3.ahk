...
class SQLite3
{
	...
	static dllPath := A_IsCompiled ? A_ScriptDir '\sqlite\lib\bin' : A_LineFile '\..\..\bin'
	...
	static __New()
	{
			...
			&& !FileExist(SQLite3.dllPath '\' SQLite3.bin)
		{
			...
			FileInstall '.\sqlite\lib\bin\sqlite332.dll', SQLite3.dllPath, true
			FileInstall '.\sqlite\lib\bin\sqlite364.dll', SQLite3.dllPath, true
		}
	...
	static open_v2(filename, &pSqlite, flags?, zVfs?)
	{
		...
		params := [{ name: 'filename', type: 'String', value: filename }, { name: 'pSqlite', type: 'Integer', value: pSqlite := 0 }, { name: 'flags', type: 'Integer', value: flags }
		]
	...
	static exec(pSqlite, statement, &errmsg, callback?, pArg?)
	{
		...
		params := [{ name: 'pSqlite', type: 'Integer', value: pSqlite }, { name: 'statement', type: 'String', value: statement }
		]
		SQLite3.check_params(params)
	...
	class Table
	{
		...
		{
			params := [{ name: 'db', type: 'SQLite', value: db }, { name: 'statement', type: 'String', value: statement }, { name: 'pTable', type: 'Integer', value: pTable }, { name: 'nRows', type: 'Integer', value: nRows }, { name: 'nCols', type: 'Integer', value: nCols }
			]
			SQLite3.check_params(params)
		...
		__Item[row, header?]
		{
			get
			{
				params := [{ name: 'row', type: 'Integer', value: row },
